...
5. Macro handling policy: do not include macro block presence or numeric macro similarity in any similarity or ecosystem health calculation. Ignore missing macro blocks and treat macro values as informational placeholders only; they must not influence pass/fail decisions. If a macro block is missing, the auditor should consider the canonical TBD placeholder (see Orchestrator) as an acceptable substitute for structural checks.
